...
namespace CarSharing.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (dbContext.db.Users.Count() == 0)
            {
                var user = new ApplicationUser { UserName = username, Email = email, Name = name, Surname = surname };
                var risp = await UserManager.CreateAsync(user, password);
                risp = UserManager.SetLockoutEnabled(user.Id, false);
                var rolesForUser = UserManager.GetRoles(user.Id);
                if (!rolesForUser.Contains(role.Name))
                {
                    risp = UserManager.AddToRole(user.Id, role.Name);
                }
            }
#endif
